...
function activate(context) {
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand("testnova.openChat", () => provider.reveal()),
...
class TestNovaSidebarProvider {
  ...
  async postActiveEditorContext(editor) {
    if (!editor) {
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .pill { border: 1px solid var(--border); border-radius: 999px; padding: 2px 8px; font-size: 10px; color: var(--muted); background: color-mix(in srgb, var(--panel) 90%, transparent); }
...
function getLoginUrl(apiBaseUrl) {
  if (!apiBaseUrl) {
